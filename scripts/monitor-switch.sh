...
if [ "$MONITOR_COUNT" -gt 1 ]; then
    ...
    hyprctl eval 'hl.monitor({ output = "DP-3", mode = "3440x1440@60", position = "1080x240", scale = 1 })'
    # 2. Enable Left Vertical (Inverted)
    hyprctl eval 'hl.monitor({ output = "HDMI-A-1", mode = "1920x1080@60", position = "0x0", scale = 1, transform = 3 })'
    # 3. Enable Right Vertical
    hyprctl eval 'hl.monitor({ output = "DP-2", mode = "1920x1080@60", position = "4520x0", scale = 1, transform = 3 })'
fi
