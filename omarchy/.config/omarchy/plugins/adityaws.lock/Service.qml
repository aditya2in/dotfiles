...
Item {
  ...
  property bool pomodoroInhibitBlank: false

  FileView {
    id: pomodoroLockStateFile
    path: (Quickshell.env("XDG_RUNTIME_DIR") || "/tmp") + "/omarchy-pomodoro-state.json"
    watchChanges: true
    printErrors: false
    onFileChanged: reload()
    onLoaded: root.checkPomodoroInhibit(text())
  }

  function checkPomodoroInhibit(raw) {
    var rawText = (typeof raw === "string") ? raw : (pomodoroLockStateFile.loaded ? pomodoroLockStateFile.text() : "")
    if (!rawText) {
