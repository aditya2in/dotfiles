...
echo "Starting Routine, Action List, and Current Task retrieval for $current_date at $(date)" | tee -a "$OUTPUT_FILE"
...
current_task_found="" # Initialize variable for the current task
...
if [ ${#matched_routines_with_lines[@]} -gt 0 ]; then
    # Print the header for currently scheduled routines
    if [ ${#matched_routines_with_lines[@]} -eq 1 ]; then
        found_routine_header="Currently scheduled Routine:"
    else
        found_routine_header="Currently scheduled Routine(s):"
    fi
    ...
    for routine_item_with_line in "${matched_routines_with_lines[@]}"; do
        ...
        echo "➡️ $routine_description" | tee -a "$OUTPUT_FILE"
...
# Now process the all_action_list_items to find the #currentTask
for task_item in "${all_action_list_items[@]}"; do
    if [[ "$task_item" =~ \#currentTask ]]; then
        current_task_found="$task_item"
        break # Exit loop after finding the first #currentTask
    fi
done

# Print the "CONSIDERATION LIST" (renamed from all_matched_tasks)
echo "" | tee -a "$OUTPUT_FILE" # Add a blank line for separation
echo "🎯 CONSIDERATION LIST (Action List):" | tee -a "$OUTPUT_FILE"
echo "----------------------------------------" | tee -a "$OUTPUT_FILE"

if [ ${#all_action_list_items[@]} -gt 0 ]; then
    for task_item in "${all_action_list_items[@]}"; do
        echo "    - $task_item" | tee -a "$OUTPUT_FILE"
    done
else
    if [ ${#matched_routines_with_lines[@]} -gt 0 ]; then
        echo "    (No specific action list items for current routines.)" | tee -a "$OUTPUT_FILE"
    else
        echo "    (No active routines or action list items to consider.)" | tee -a "$OUTPUT_FILE"
    fi
fi
echo "--------------------------------------------------------" | tee -a "$OUTPUT_FILE"
...
echo "" | tee -a "$OUTPUT_FILE" # Add a blank line for separation
echo "⚡ CURRENT TASK:" | tee -a "$OUTPUT_FILE"
echo "----------------------------------------" | tee -a "$OUTPUT_FILE"

if [ -n "$current_task_found" ]; then
    echo "    - $current_task_found" | tee -a "$OUTPUT_FILE"
else
    echo "    (No current task identified with '#currentTask' tag.)" | tee -a "$OUTPUT_FILE"
fi
echo "--------------------------------------------------------" | tee -a "$OUTPUT_FILE"
