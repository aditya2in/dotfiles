...
DAILY_LOG_FILE="$POMODORO_DIR/pomodoro_daily_log.txt" # <--- NEW CONFIG VARIABLE
...
handle_transition() {
    ...
    if [ "$_session_type" == "Work" ]; then
        # This branch: Work session just finished. Automatically start a Break.
        ...
        if [ "$_current_session_in_cycle" -eq 4 ]; then
            ...
            start_session "Long Break" "$LONG_BREAK_DURATION" "Long Break Time!" "Enjoy your ${LONG_BREAK_DURATION%[sm]} ${unit_label} long break!"
        else
            # It's time for a Short Break
            start_session "Break" "$SHORT_BREAK_DURATION" "Short Break Time!" "Take a ${SHORT_BREAK_DURATION%[sm]} ${unit_label} break."
        fi
    elif [ "$_session_type" == "Break" ] || [ "$_session_type" == "Long Break" ]; then
        # This branch: Break session just finished. Do NOT automatically start Work.
        # Instead, set state to Stopped/None and notify user to manually start Work.
        if [ "$_current_session_in_cycle" -eq 4 ]; then
...
# --- END handle_transition MODIFIED BLOCK ---
...
cmd_start() {
    ...
    start_session "Work" "$WORK_DURATION" "Pomodoro Started!" "Time to focus for ${WORK_DURATION%[sm]} ${unit_label}."
}

# Command to pause the Pomodoro timer.
cmd_pause() {
    read_state
    if [ "$_status" == "Stopped" ]; then # Changed from "!=" to "==" for clearer logic.
...
# --- MODIFIED BLOCK BELOW: cmd_reset logic to preserve counts robustly ---
...
cmd_status() {
    ...
    # Get pomodoro-cli's JSON output (captures stdout, redirects stderr)
    local pcli_json=$(pomodoro-cli status --format json --time-format digital 2>/dev/null)
    ...
    local full_text="[[${_total_pomodoro_cycles_today}]] ${_status}/${_session_type} ${time_text} (${current_cycle_display})"
    ...
    echo "DEBUG: percentage_val (before final tonumber): '$percentage_val'" >&2
...
cmd_daemon() {
    ...
    if [ -f "$DAEMON_PID_FILE" ] && kill -0 "$(cat "$DAEMON_PID_FILE")" 2>/dev/null; then
        echo "Pomodoro daemon is already running (PID: $(cat "$DAEMON_PID_FILE"))." >&2 # Redirected to stderr
        exit 0
    fi

    echo $$ > "$DAEMON_PID_FILE" # Store current PID

    echo "DEBUG: Pomodoro daemon started. Monitoring sessions..." >&2 # Added DEBUG
    ...
    while true; do
        ...
        if [ "$_status" == "Running" ]; then
            # Get pomodoro-cli's JSON output
            local pcli_status_json=$(pomodoro-cli status --format json 2>/dev/null)
            ...
            if [[ "$pcli_status_class" == "finished" ]]; then
                echo "DEBUG: Daemon detected pomodoro-cli 'finished' class. Triggering transition." >&2 # Added debug
                handle_transition
            elif [[ "$pcli_status_class" == "stopped" ]]; then # Also check for 'stopped' class, as a fallback
                echo "DEBUG: Daemon detected pomodoro-cli 'stopped' class while script state is 'Running'. Triggering transition." >&2 # Added debug
                handle_transition
            fi
        fi
...
cmd_stop_daemon() {
    if [ -f "$DAEMON_PID_FILE" ]; then
        ...
        if kill -TERM "$pid" 2>/dev/null; then
            echo "Pomodoro daemon (PID: $pid) stopped." >&2 # Redirected to stderr
            rm -f "$DAEMON_PID_FILE"
        else
            echo "Failed to stop daemon (PID: $pid). It might not be running or permission denied." >&2
        fi
    else
        echo "Pomodoro daemon not running (PID file not found)." >&2 # Redirected to stderr
    fi
}

# Main script logic: parse arguments
# Ensure the state file exists and is initialized on script startup if needed.
# This will only create the file if it doesn't exist, is empty, or is invalid JSON.
# The `initialize_state` call at the bottom has been removed as `read_state` now handles this.
# initialize_state  <--- THIS LINE IS REMOVED
...
case "$1" in
    ...
    stop-daemon)
        ...
        ;;
    *)
        echo "Usage: $0 {start|pause|resume|stop|reset|status|daemon|stop-daemon}" >&2 # Redirected to stderr
        exit 1
        ;;
esac
