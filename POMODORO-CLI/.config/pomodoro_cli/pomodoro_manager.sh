...
OBSIDIAN_VAULT_PATH="$HOME/obsidian" # Assuming your Obsidian vault is directly in your home directory
OBSIDIAN_VAULT_NAME="obsidian" # <--- CONFIRM/REPLACE with your actual Obsidian vault name (e.g., "My Notes")
OBSIDIAN_BREAK_NOTE_PATH="$OBSIDIAN_VAULT_PATH/All Things/Journal/Pomodoro session records/POMODORO BREAK FILE.md" # <--- IMPORTANT: This is the file you provided
...
reset_daily_counts() {
    ...
    if [ "$_last_run_date" != "$today" ]; then
        echo "DEBUG: New day detected. Logging previous day's summary and resetting state." >&2
        ...
        echo "DEBUG: Resetting state for new day." >&2
...
start_session() {
    ...
    if [ "$type" == "Break" ] || [ "$type" == "Long Break" ]; then
        echo "DEBUG: Triggering break action for "$type": New window management sequence." >&2
        ...
        echo "DEBUG: Opening Obsidian URI: ${obsidian_break_uri}" >&2
        nohup xdg-open "${obsidian_break_uri}" >/tmp/pomodoro_obsidian_output.log 2>&1 &
        echo "DEBUG: Obsidian break file launched via xdg-open." >&2
        ...
        echo "DEBUG: Captured Obsidian break window address: $_obsidian_break_window_address" >&2
        ...
        if [ -n "$_obsidian_break_window_address" ]; then
            ...
            echo "DEBUG: Moved Obsidian to Workspace 1." >&2
            ...
            echo "DEBUG: Moved Obsidian to Workspace 9." >&2
            sleep 0.5 # Give Hyprland time to process
            ...
            echo "DEBUG: Fullscreened Obsidian on Workspace 9." >&2

            # --- NEW: Use the frequent locker during breaks ---
            local duration_in_seconds=$(parse_duration_to_seconds "$duration_str")
            /home/aditya/dotfiles/POMODORO-CLI/.config/pomodoro_cli/break_frequent_locker.sh "$duration_in_seconds" &
            echo "DEBUG: Started frequent screen lock for the break duration." >&2
            # --- END NEW ---
        else
...
cmd_daemon() {
    ...
    while true; do
        ...
        if [ "$_status" == "Running" ]; then
            ...
            echo "DEBUG: Daemon: Remaining seconds: $remaining_seconds" >&2
            if (( remaining_seconds > 0 && remaining_seconds <= 5 )); then
                echo "DEBUG: Playing 5-second warning sound (continuous)." >&2
                play_sound
            fi
        fi
