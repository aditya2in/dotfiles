...
start_session() {
    ...
    if [ "$type" == "Break" ] || [ "$type" == "Long Break" ]; then
        ...
        if [ -n "$_obsidian_break_window_address" ]; then
            ...
            # --- NEW: Wait for the configured delay and then lock the screen with hyprlock ---
            (sleep "$BREAK_LOCK_DELAY_SEC" && hyprlock) &
            echo "DEBUG: Screen will lock in $BREAK_LOCK_DELAY_SEC seconds using hyprlock." >&2
            # --- END NEW ---
        else
...
cmd_daemon() {
    ...
    while true; do
        ...
        if [ "$_status" == "Running" ]; then
            ...
            local remaining_seconds=$(echo "$pcli_status_json" | jq -r '.remaining // 0 | tonumber')
